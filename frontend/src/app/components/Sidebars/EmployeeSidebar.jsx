import { useState } from "react";
import {
  MdDashboard,
  MdAccountCircle,
  MdWork,
  MdCalendarToday,
  MdReceipt,
  MdKeyboardArrowLeft,
  MdKeyboardArrowRight,
  MdMenu,
} from "react-icons/md";
import { Link } from "react-router-dom";

export default function EmployeeSidebar() {
  const [open, setOpen] = useState(true);
  const [mobileOpen, setMobileOpen] = useState(false);

  return (
    <>
      <button
        className="md:hidden fixed top-4 left-4 z-50 p-3 bg-white shadow rounded-full"
        onClick={() => setMobileOpen(!mobileOpen)}
      >
        <MdMenu className="text-2xl text-[#DB5E00]" />
      </button>

      <div
        className={`
          h-screen bg-white shadow-lg border-r border-gray-200 flex flex-col transition-all duration-300
          ${open ? "w-80" : "w-24"}

          /* FIXED ON MOBILE */
          fixed top-0 left-0 z-40

          /* STICKY ON DESKTOP */
          md:sticky md:top-0

          ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}
        `}
      >
        <div className="pt-8 pb-6 px-4 flex flex-col items-center relative">
          {open && (
            <img
              src="/kacha-logo.jpg"
              alt="Kacha Logo"
              className="h-14 w-auto object-contain mx-auto"
            />
          )}

          <button
            onClick={() => setOpen(!open)}
            className="hidden md:flex absolute right-3 top-3 p-2 bg-gray-100 rounded-full hover:bg-gray-200 shadow"
          >
            {open ? <MdKeyboardArrowLeft /> : <MdKeyboardArrowRight />}
          </button>
        </div>

        <div className="flex-1 mt-4 px-4 space-y-2 overflow-y-auto">
          <Link
            to="/employee/dashboard"
            className="group flex items-center gap-5 p-4 rounded-lg text-gray-700 font-semibold text-lg transition hover:bg-[#FFCC00] hover:text-white"
            onClick={() => setMobileOpen(false)}
          >
            <MdDashboard className="text-3xl text-[#DB5E00] group-hover:text-white" />
            {open && <span>Dashboard</span>}
          </Link>

          <Link
            to="/employee/profile"
            className="group flex items-center gap-5 p-4 rounded-lg text-gray-700 font-semibold text-lg transition hover:bg-[#FFCC00] hover:text-white"
            onClick={() => setMobileOpen(false)}
          >
            <MdAccountCircle className="text-3xl text-[#DB5E00] group-hover:text-white" />
            {open && <span>My Profile</span>}
          </Link>

          <Link
            to="/employee/tasks"
            className="group flex items-center gap-5 p-4 rounded-lg text-gray-700 font-semibold text-lg transition hover:bg-[#FFCC00] hover:text-white"
            onClick={() => setMobileOpen(false)}
          >
            <MdWork className="text-3xl text-[#DB5E00] group-hover:text-white" />
            {open && <span>My Tasks</span>}
          </Link>

          <Link
            to="/employee/leave"
            className="group flex items-center gap-5 p-4 rounded-lg text-gray-700 font-semibold text-lg transition hover:bg-[#FFCC00] hover:text-white"
            onClick={() => setMobileOpen(false)}
          >
            <MdCalendarToday className="text-3xl text-[#DB5E00] group-hover:text-white" />
            {open && <span>Leave Request</span>}
          </Link>

          <Link
            to="/employee/payslip"
            className="group flex items-center gap-5 p-4 rounded-lg text-gray-700 font-semibold text-lg transition hover:bg-[#FFCC00] hover:text-white"
            onClick={() => setMobileOpen(false)}
          >
            <MdReceipt className="text-3xl text-[#DB5E00] group-hover:text-white" />
            {open && <span>Payslips</span>}
          </Link>
        </div>
      </div>
    </>
  );
}
