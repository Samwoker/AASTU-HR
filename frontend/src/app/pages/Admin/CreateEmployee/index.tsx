...
export default function CreateEmployee() {
  ...
  const resolveJobTitle = (name: string): number | null => {
    ...
    // Simple lookup:
    const existing = jobTitles.find(j => j.title.toLowerCase() === name.toLowerCase());
    return existing ? existing.id : null;
  };

  const resolveAllowanceType = (name: string): number | null => {
    const existing = allowanceTypes.find(a => a.name.toLowerCase() === name.toLowerCase());
    return existing ? existing.id : null;
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.email || !form.fullName || !form.grossSalary || !form.departmentName || !form.jobTitleName) {
    ...
    try {
      ...
      if (!deptId) return ToastService.error(`Department "${form.departmentName}" not found. Please create it first.`);
      
      const jobId = resolveJobTitle(form.jobTitleName);
      if (!jobId) return ToastService.error(`Job Title "${form.jobTitleName}" not found. Please create it first.`);
      ...
      for (const a of allowances) {
        if (a.name && a.amount) {
           ...
           if (!typeId) {
             ToastService.error(`Allowance Type "${a.name}" not found. Please create it first.`);
             return;
           }
  ...
  if (createModalState.type === "department") {
    ...
    modalFields = [
      { name: "name", label: "Allowance Name", required: true, defaultValue: createModalState.initialValue },
      { name: "description", label: "Description", required: true, type: "textarea", placeholder: "e.g. Monthly transport allowance" },
    ...
    <AdminLayout>
      <div className="w-full px-6 pb-12"> 
        ...
        <div className="mb-8">
          ...
          />
          <h1 className="text-3xl font-bold text-k-dark-grey">Add New Employee</h1>
          <p className="text-k-medium-grey mt-2">
            Send an invite to a new employee. They will set up their password and personal details later.
          </p>
        </div>
