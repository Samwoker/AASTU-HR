...
export default function CreateEmployment() {
  ...
  useEffect(() => {
    const fetchEmployment = async () => {
        ...
        try {
            ...
            const emp = response?.data?.employment || response?.data?.data?.employment;
            if (!emp) throw new Error("Employment not found");

            const dept = departments.find(d => d.id === (emp.department_id ?? emp.department?.id)) || 
                         (emp.department ? { name: emp.department.name } : null);
            
            const job = jobTitles.find(j => j.id === (emp.job_title_id ?? emp.jobTitle?.id)) ||
            ...
            setForm(prev => ({
                ...
                start_date: emp.start_date ? emp.start_date.split('T')[0] : prev.start_date,
            ...
            if (emp.allowances && Array.isArray(emp.allowances)) {
                emp.allowances.forEach((a: any) => {
                   const typeName = a.allowanceType?.name || allowanceTypes.find(at => at.id === a.allowance_type_id)?.name;
                   if (typeName) {
                       mappedAllowances.push({ name: typeName, amount: String(a.amount) });
                   }
                });
            } else {
                // Fallback to legacy fields if backend returns them
                if (Number(emp.transportation_allowance) > 0) 
                    mappedAllowances.push({ name: "Transport Allowance", amount: String(emp.transportation_allowance) });
                if (Number(emp.housing_allowance) > 0) 
                    mappedAllowances.push({ name: "Housing Allowance", amount: String(emp.housing_allowance) });
                if (Number(emp.meal_allowance) > 0) 
                    mappedAllowances.push({ name: "Meal Allowance", amount: String(emp.meal_allowance) });
            }
    ...
    if (employmentId && departments.length > 0) { // Wait for dropdowns to be ready to map IDs to Names
        fetchEmployment();
  ...
  const handleAllowanceChange = (index: number, field: keyof AllowanceEntry, value: string) => {
  ...
  const removeAllowance = (index: number) => {
   ...
   // Filters
  const filterDepartments = async (query: string) => departments.filter(d => d.name.toLowerCase().includes(query.toLowerCase())).map(d => d.name);
  const filterJobTitles = async (query: string) => jobTitles.filter(j => j.title.toLowerCase().includes(query.toLowerCase())).map(j => j.title);
  const filterAllowanceTypes = async (query: string) => allowanceTypes.filter(a => a.name.toLowerCase().includes(query.toLowerCase())).map(a => a.name);

  // Creation
   const handleOpenCreateModal = (type: "department" | "jobTitle" | "allowance", value: string, index?: number) => {
    setCreateModalState({ isOpen: true, type, initialValue: value, targetIndex: index });
  };
  ...
  const handleCreateSubmit = async (values: Record<string, any>) => {
      try {
        if (createModalState.type === "department") {
            ...
            const newJob = await adminService.createJobTitle(values.title, values.level);
            ...
            const newAllowance = await adminService.createAllowanceType({
                name: values.name, description: values.description, is_taxable: isTaxable
            });
            setAllowanceTypes(prev => [...prev, newAllowance]);
            if (typeof createModalState.targetIndex === 'number') {
                handleAllowanceChange(createModalState.targetIndex, "name", newAllowance.name);
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      e.preventDefault();
      if (!form.departmentName || !form.jobTitleName || !form.gross_salary || !form.employee_id) {
          return ToastService.error("Please fill required fields");
      }

      const deptId = departments.find(d => d.name.toLowerCase() === form.departmentName.toLowerCase())?.id;
      if (!deptId) return ToastService.error("Department not found. Please create it first.");
      
      const jobId = jobTitles.find(j => j.title.toLowerCase() === form.jobTitleName.toLowerCase())?.id;
      if (!jobId) return ToastService.error("Job Title not found. Please create it first.");

      const validAllowances = [];
      for (const a of allowances) {
        if (a.name && a.amount) {
           const typeId = allowanceTypes.find(at => at.name.toLowerCase() === a.name.toLowerCase())?.id;
           if (!typeId) return ToastService.error(`Allowance "${a.name}" not found.`);
      ...
      if (Math.abs(gross - (basic + allowanceTotal)) > 0.01) {
          return ToastService.error(`Compensation Mismatch: Base Salary (${basic}) + Allowances (${allowanceTotal}) must equal Gross Salary (${gross}). Difference: ${gross - (basic + allowanceTotal)}`);
      }
      ...
      return (
          <AdminLayout>
             <div className="flex h-[80vh] items-center justify-center">
                 <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-k-orange"></div>
             </div>
          </AdminLayout>
      );
  }
  ...
  if (createModalState.type === "department") {
      ...
      modalFields = [{ name: "name", label: "Department Name", required: true, defaultValue: createModalState.initialValue }];
  } else if (createModalState.type === "jobTitle") {
      modalTitle = "Create New Job Title";
      modalFields = [
        { name: "title", label: "Job Title", required: true, defaultValue: createModalState.initialValue },
        { name: "level", label: "Level", type: "select", required: true, options: [
            { label: "Junior", value: "Junior" }, { label: "Mid", value: "Mid" }, { label: "Senior", value: "Senior" }
        ]}
      ];
  } else if (createModalState.type === "allowance") {
      modalTitle = "Create Allowance Type";
      modalFields = [
          { name: "name", label: "Name", required: true, defaultValue: createModalState.initialValue },
    ...
    <AdminLayout>
      <div className="w-full px-6 pb-12">
        <div className="mb-8">
           ...
           />
           <h1 className="text-3xl font-bold text-k-dark-grey">{employmentId ? "Update Employment" : "Assign Employment"}</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-card p-8 border border-gray-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormAutocomplete
                        ...
                        onChange={(val) => handleAutocompleteChange("jobTitleName", val)}
                    ...
                    <FormAutocomplete
                        ...
                        onChange={(val) => handleAutocompleteChange("departmentName", val)}
            ...
            <div className="bg-white rounded-2xl shadow-card p-8 border border-gray-100">
                ...
                <div className="border-t pt-6">
                    ...
                    <div className="space-y-4 mb-4">
                        {allowances.map((allowance, index) => (
                            <div key={index} className="flex gap-4 items-end bg-gray-50 p-3 rounded-xl border border-gray-200">
                                <div className="flex-1">
                                    <FormAutocomplete
                                        ...
                                        onCreateNew={(val) => handleOpenCreateModal("allowance", val, index)}
             ...
             <div className="flex justify-end pt-6">
                 ...
                 <Button type="submit" loading={loading || isUpdating} className="w-40">
        ...
        <CreateItemModal 
            ...
            initialValues={{ name: createModalState.initialValue, title: createModalState.initialValue }}
        />
      </div>
    </AdminLayout>
  );
